...
export const VolumeChart: React.FC<VolumeChartProps> = ({ activeMetrics, days, timeseries }) => {
    ...
    const data = useMemo(() => {
        if (timeseries && timeseries.length > 0) {
            return timeseries.map(item => {
                ...
                return {
                    name: label,
                    messages: item.total_messages || (item.sessions * item.avg_messages),
                    tokens: item.tokens,
                    escalation: item.handoff_rate,
