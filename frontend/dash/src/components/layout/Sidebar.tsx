        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
                <span className="font-label-sm text-[13px] text-text-primary text-ellipsis">{selectedChatbot?.name || ''}</span>
              </div>
            </div>
                ...
                {chatbots.map(bot => (
                  <div 
                    ...
                    className={`px-3 py-2 mx-1.5 rounded-lg text-[13px] font-body-base cursor-pointer flex items-center gap-2 transition-colors
                      ${bot.id === selectedChatbot?.id ? 'bg-primary/10 text-primary' : 'text-on-surface-variant hover:bg-surface-glass hover:text-text-primary'}
                    `}
                  >
        ...
        {NAV_ITEMS.map((item, idx) => (
          <li key={idx}>
            <NavLink
              ...
              className={({ isActive }) => `flex items-center gap-3 py-3 font-label-sm text-label-sm transition-all duration-200 border-l-4 ${
                isActive
