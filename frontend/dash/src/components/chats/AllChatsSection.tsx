  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      const fetchChats = async () => {
        try {
          const res = await apiJson('/api/chats/search', 'POST', { keyword: searchQuery });
          if (res.ok) {
            const data = await res.json();
            const formattedChats: ChatSession[] = data.map((item: any) => ({
              id: item.session_id,
              name: `Session ${item.session_id.substring(0, 6)}`,
              source: 'Website',
              status: 'Resolved',
              preview: item.snippet,
              date: item.date,
              messages: (item.messages || []).map((m: any, idx: number) => ({
                id: `m${idx}`,
                sender: m.role === 'model' || m.role === 'bot' ? 'bot' : 'user',
                text: m.content,
                time: item.date
              }))
            }));
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border-subtle">
                {filteredChats.length === 0 && (
                  <tr>
                    <td colSpan={4} className="py-12 text-center text-text-muted font-body-base">
