...
def list_integrations():
    ...
    cur.execute("SELECT id, file_id, file_name, file_type, sheet_range, cached_content, last_synced_at FROM knowledge_bases WHERE client_id = %s ORDER BY last_synced_at DESC", (session['client_id'],))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    ...
    results = []
    for row in rows:
        db_id, file_id, file_name, file_type, sheet_range, cached_content, last_synced_at = row
        ...
        date_modified = last_synced_at.strftime("%b %d, %Y") if last_synced_at else "Unknown"
        ...
        results.append({
            ...
            "dateAdded": date_modified # We'll just use the same for dateAdded for now
        })
...
def get_integrations_status():
    ...
    try:
        cur.execute(
            "SELECT google_token FROM clients WHERE client_id = %s",
            (session['client_id'],)
        )
        row = cur.fetchone()
        google_connected = bool(row and row[0])

        return jsonify({
            "google": {"connected": google_connected, "name": "Google Workspace"},
            "slack": {"connected": False, "name": "Slack", "coming_soon": True},
            "shopify": {"connected": False, "name": "Shopify", "coming_soon": True},
            "zendesk": {"connected": False, "name": "Zendesk", "coming_soon": True}
        })
    except Exception as e:
        logging.error(f"Integrations Status Error: {str(e)}")
        return jsonify({"error": str(e)}), 500
    finally:
        cur.close()
        conn.close()