...
@chat_bp.route('/api/chats/live', methods=['GET'])
def get_live_chats():
    if 'client_id' not in session: return jsonify({"error": "Unauthorized"}), 401
    ...
    try:
        cur.execute("""
            ...
            FROM chat_sessions 
            WHERE client_id = %s AND updated_at > NOW() - INTERVAL '20 minutes'
            ORDER BY updated_at DESC LIMIT 20
        """, (session['client_id'],))
        
        results = []
        for row in cur.fetchall():
            session_id, updated_at, messages = row
            
            time_diff = datetime.now(timezone.utc) - updated_at.replace(tzinfo=timezone.utc)
            mins, secs = divmod(int(time_diff.total_seconds()), 60)
            time_active = f"{mins:02d}:{secs:02d}"
