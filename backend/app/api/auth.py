...
def get_access_token():
    ...
    cur = conn.cursor()
    cur.execute("SELECT google_token, client_id_google FROM clients WHERE client_id = %s", (session['client_id'],))
    row = cur.fetchone()
    cur.close()
    conn.close()
    if row and row[0]: return jsonify({"token": row[0], "clientId": row[1]})
